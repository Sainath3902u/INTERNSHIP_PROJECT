...
def packet_stateless__count(real_df_1, gen_df_2, n=10):
    ...
    Note:
        ...
        SELECT COUNT(*) AS total_packets FROM <table_name>

        Therefore, each DataFrame contains a single row with the
        'total_packets' column instead of raw packet-level records.
    """

    # Extract packet counts from the SQL aggregation results
...
def packet_stateless_srcip_countdistinct(real_df_1, gen_df_2, n=10):
    ...
    Note:
        The inputs are SQL aggregation results generated from:
        SELECT COUNT(DISTINCT srcip) AS n_src_ips FROM {table_name}

        Therefore, each DataFrame contains a single row with the
        'n_src_ips' column instead of raw packet-level records.
    """

    # Extract distinct source IP counts from the SQL aggregation results
    real_count = real_df_1['n_src_ips'].iat[0]
    gen_count = gen_df_2['n_src_ips'].iat[0]
...
def packet_stateless_pktlen_distribution(real_df_1, gen_df_2, n=10):
    ...
    return jensenshannon_wrapper(real_aligned.values, gen_aligned.values)
...
def flow_dstport_stateless_bytes_distribution(real_df_1, gen_df_2, n=10):
    ...
    return jensenshannon_wrapper(real_aligned.to_numpy(), gen_aligned.to_numpy())
